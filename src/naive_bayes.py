...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        for i in range(len(unique_classes)):
            class_priors[int(unique_classes[i])] = counts[i]/total
    ...
    def estimate_conditional_probabilities(
        ...
        for unique in unique_classes:
            class_features = features[labels == unique]
            counts = class_features.sum(dim=0)
            
            total = counts.sum()
            
            prob = (delta + counts) / (delta * size + total)
            
            class_word_counts[int(unique)] = prob
    ...
    def estimate_class_posteriors(
        ...
        for unique, prior in self.class_priors.items():
            prior_log = torch.log(prior)
            cond_prob_log = torch.log(self.conditional_probabilities[unique])
            likelihood_log = torch.sum(feature * cond_prob_log)
            
            log_posteriors[unique] = prior_log + likelihood_log
